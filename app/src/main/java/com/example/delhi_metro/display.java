...
HashMap<String,Integer>color=new HashMap<String,Integer>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     set.add("Central Secretariat");
        ...
        result = findViewById(R.id.textView44);
        ...
        if(source!=destination) {
            ...
            String in="(Interchange)";
            SpannableString str = new SpannableString(res);
            int st = 0;
            for (int i = 0; i < ans.size(); i++){
                if(!set.contains(ans.get(i)))
                {str.setSpan(new ForegroundColorSpan(color.get(ans.get(i))), st, st + ans.get(i).length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                st = st + ans.get(i).length() + 1;}
                else{
                    st=st+ans.get(i).length()+in.length()+1;
                }
            }
            result.setText(str);
        }
        ...
        { fare=-1;
        duration=-1;
            result.setText("ERROR IN THE STATEMENT !!!!!!");
        }
